...
class CoreVulpes(ABC):
    ...
    def missing_data(self, X: Array_like) -> pd.DataFrame:
        ...
        percent_missing = ((X.isnull().sum() / X.isnull().count())
    ...
    def create_pipeline(self, model_name: str, model: Any) -> Pipeline:
        ...
        steps = reduce(operator.add,
                       [p.steps for p in pipelines if not(p is None)])
        return Pipeline(steps=steps)

    def predefined_cv(self, X: Array_like = None,
                      groups: Array_like = None) -> Any:
    ...
    def build_best_models(self, X: Array_like, y: Array_like, *,
                          ...
                          ascending: bool = None,
                          voting: str = "hard",
                          weights: Array_like = None) -> pd.DataFrame:
        ...
        if not(sample_weight is None):
            raise NotImplementedError(
                "Sample weight not (yet) implemented when fitting a"
                " 'best model'.")
        fit_params = {}
        ######
    ...
    def get_fitted_models(self) -> Dict[str,
    ...
    def predict(self, X: Array_like, *,
        ...
        for name, fitted_model in fitted_models.items():
            if (isinstance(fitted_model, List) or
               isinstance(fitted_model, np.ndarray)):
                print("Cross validation used to fit. Select the model"
    ...
    def predict_best(self, X: Array_like) -> np.ndarray:
        ...
        if (isinstance(best_model, List) or
           isinstance(best_model, np.ndarray)):
            print("Cross validation used to fit. Select the model"
                  " fitted on the first fold.")
